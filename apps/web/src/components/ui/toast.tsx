...
function Toast({
  ...
  return (
    <ToastPrimitive.Root
      ...
      className={cn(
        [
          "group/toast pointer-events-auto absolute end-0 bottom-0",
          "z-[calc(1000-var(--toast-index))] w-full origin-bottom",
          ...
          "[--shrink:calc(1-var(--scale))] h-(--height)",
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  if (type === "success") {
    return <CheckCircle2 className={cn(iconClass, "text-success")} aria-hidden="true" />
  }
  if (type === "info") {
    return <Info className={cn(iconClass, "text-info")} aria-hidden="true" />
  }
  if (type === "warning") {
    return <TriangleAlert className={cn(iconClass, "text-warning")} aria-hidden="true" />
  }
  if (type === "error") {
    return <XCircle className={cn(iconClass, "text-destructive")} aria-hidden="true" />
  }
  if (type === "loading") {
    return <LoaderCircle className={cn(iconClass, "animate-spin text-primary")} aria-hidden="true" />
  }
  return null
}
